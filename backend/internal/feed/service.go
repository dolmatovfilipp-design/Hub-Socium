...
// Interesting returns popular/explore-style posts (not limited to following).
func (s *Service) Interesting(w http.ResponseWriter, r *http.Request) {
	uid, ok := apiutil.UserIDFromContext(r.Context())
	if !ok {
		apiutil.Error(w, http.StatusUnauthorized, "unauthorized", "missing user")
		return
	...
	q := `
		...
		WHERE p.deleted_at IS NULL
		  ...
		  AND p.author_id NOT IN (SELECT muted_id FROM mutes WHERE muter_id = $1)
		  AND (
		    COALESCE(u.is_private,false) = false
		    OR p.author_id = $1
		    OR EXISTS(SELECT 1 FROM follows f WHERE f.follower_id = $1 AND f.followee_id = p.author_id)
		  )`
	args := []any{uid}
	argN := 2
	if tag != "" {
	...
	if hasCursor {
		// Score cursor: likes desc, then created_at, id — use created_at/id for simplicity after score sort page
		q += fmt.Sprintf(` AND (p.created_at, p.id) < ($%d::timestamptz, $%d::uuid)`, argN, argN+1)
		args = append(args, cursorCreated, cursorID)
		argN += 2
	}
	...
	for rows.Next() {
		...
		item := map[string]any{
			...
			"created_at": created.UTC().Format(time.RFC3339Nano),
			"likes": likes, "comments": comments, "liked_by_me": likedByMe,
			"reposts": reposts, "reposted_by_me": repostedByMe,
		}
		if imageURL != "" {
			item["image_url"] = imageURL
		}
	...
	apiutil.JSON(w, http.StatusOK, map[string]any{"items": items, "next_cursor": next, "mode": "interesting"})
}
